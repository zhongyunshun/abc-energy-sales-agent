...
> Prerequisites: M0, M3 products (or fixtures), container GPU available (T0.8)

## Tasks

- [x] T4.1 Implement `training/formatting.py`: `DialogueRecord` → Qwen chat template rendering + completion-only masking boundaries (confirm the API for the locked TRL version, see the masking decision) + unit tests (assistant-span start/end positions, multi-turn masking correctness)
- [x] T4.2 Author `configs/sft.yaml`: put the full hyperparameter baseline table from the M4 baseline into config (LoRA r16/α32, lr 2e-4 cosine, bs 2×8, seq 2048, adamw_8bit, gradient_checkpointing)
